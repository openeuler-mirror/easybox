...
use libc::{sockaddr, sockaddr_storage, EINVAL, ETH_ALEN};

use crate::net_tools::HWType;

pub const ETHER_HWTYPE: HWType = HWType {
    name: "ether",
    title: "Ethernet",
    typ: libc::ARPHRD_ETHER as i32,
    alen: libc::ETH_ALEN,
    print: Some(print),
    input: Some(hinput),
    activate: None,
...
pub fn hinput(bufp: &str, sasp: &mut sockaddr_storage) -> Result<(), i32> {
    ...
    sap.sa_family = libc::ARPHRD_ETHER;

    let mut i: usize = 0;
    let mut chars = bufp.chars().peekable();
    while let Some(c) = chars.next() {
        if i >= ETH_ALEN as usize {
            break;
        }
