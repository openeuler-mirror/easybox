...
const MD5SUM_CMD_PARSE_ERROR: i32 = 1;
...
///
pub fn parse_md5sum_cmd_args(args: impl uucore::Args, about: &str, usage: &str) -> UResult<Config> {
    let command = md5sum_app(about, usage);
    let arg_list = args.collect_lossy();
    Config::from(&command.try_get_matches_from(arg_list)?)
}

///
pub fn md5sum_app<'a>(about: &'a str, usage: &'a str) -> Command<'a> {
...
pub fn check_hash<R: Read>(input: &mut R, ref_hash: &str) -> UResult<bool> {
    ...
    let hash = md5_reader(reader);
    let core_wrapper = hash?;
    let final_result = core_wrapper.finalize();
    let mut hex_hash = String::new();
    for byte in final_result.iter() {
        write!(hex_hash, "{:02x}", byte).expect("Unable to write");
    }
...
pub fn parse_check_line(input: &str, config: &Config) -> UResult<(String, String, i32)> {
    ...
    if config.warn && !check_hash_format(hash)? {
        eprintln!(
            "WARNING: {}: no properly formatted md5 checksum lines found",
            filename
        );
        if config.strict {
            ret_code = 1;
...
/// Read the file and calculate the hash value
pub fn md5_reader<R: Read>(mut input: R) -> UResult<Md5> {
